...
現在は全対面記事DBを運用する段階です。検証済みの記事は優先表示し、未執筆または記事取得に失敗した対面はRiot Data Dragonのチャンピオン属性、個別スキル名、スキル説明から下書き品質の判断基準を生成します。パッチ別勝率、OTP向けの細かいコンボ、統計サイト由来の最適ビルドは今後追加する想定です。
